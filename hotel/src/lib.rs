...
unsafe extern "C" fn hard_fault_handler() {
    ...
    // This assembly stores the faulting stack pointer
    // in r0 and stores whether it is a kernel stack in r1
    // (1 is kernel, 0 is user). It does this by checking
    // the value in the link register, then depending on this
    // either copying the master stack pointer (msp) or the
    // process stack pointer (psp) into r0.
    asm!(
        "mov    r1, 0                       \n\
         tst    lr, #4                      \n\
         itte   eq                          \n\
